...
namespace RSM.Support.S2
{
	...
	{
		private RSMDataModelDataContext db;
		private S2API _api;
		...
		public int ImportLevels()
		{
			...
			if (IDs != null)
			{
				...
				foreach (var ID in IDs)
				{
					...
					if (details == null)
					{
						EventLog.WriteEntry("R1SM",
						                    string.Format("S2 returned no details for access level {0}.\n{1}", ID, _api.LastResponse),
					...
					try
					{
						...
						{
							db.Syslog(ImportSystem,
							          Severity.Warning, 
							          string.Format("S2 access level \"{0}\" renamed to \"{1}\".", level.AccessLevelName,
							                        details["ACCESSLEVELNAME"].InnerText),
