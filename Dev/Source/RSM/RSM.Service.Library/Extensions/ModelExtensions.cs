...
namespace RSM.Service.Library.Extensions
{
	...
	public static class ModelExtensions
	{
			...
			try
			{
				using (var db = new RSMDB.RSMDataModelDataContext())
				{
					using (var transaction = new TransactionScope(TransactionScopeOption.Required, TransactionTimeout))
					{
						...
						var keys = (from as ExternalEntity).Insert(db);
						if (keys == null)
        ...
        public static Result<Person> Update(this Person from)
        {
            var result = Result<Person>.Success();
            ...
            try
            {
                using (var db = new RSMDB.RSMDataModelDataContext())
                {
                    using (var transaction = new TransactionScope(TransactionScopeOption.Required, TransactionTimeout))
                    {
                        ...
                        var keys = (from as ExternalEntity).Select(db);
                        if (keys == null)
                            return result.Fail("Update Person failed to load keys");
