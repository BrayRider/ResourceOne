...
{
	public class NewHires : DataController<NewHire>
	{
		public Result<List<NewHire>> Search(PagedRequest request, Expression<Func<NewHire, bool>> filterExpression)
		{
			var results = new Result<List<NewHire>>(new Dictionary<string, string>());

			DbContext.DeferredLoadingEnabled = false;
			var loadOptions = new DataLoadOptions();
			DbContext.LoadOptions = loadOptions;

			var query = DbContext.NewHires.AsQueryable();
